...
function renderProduct(product) {
    let contents = `
        <tr>
            ...
            <td>${product.idcategory === "0" ? "Men's shoes" : product.idcategory === "1" ? "Women's shoes" : "Accessory"}</td>
...
function actAddProduct() {
    ...
    if ((validateForm(product) === true) && (checkExistProductCode(product.code) === false)) {
        ...
        localStorage.setItem('DATABASE', JSON.stringify(DATABASE));
        renderProduct(product);
        notificationAction("More Successful Products.", "#12e64b");
...
function validateForm(product) {
    if (product.code === "" || product.productName === ""
        ...
        notificationAction("Please complete all information.", "#e61212");
...
function checkExistProductCode(code) {
    ...
    PRODUCTS.forEach(function (product) {
        if (product.code === code) {
            notificationAction("Product Code Existing.", "#e61212");
            add_new.disabled = true;
            check++;
        }
    })
...
function actProduct(event) {
    ...
    if (ev.matches('#detail')) {
        ...
        let modals = `
                ...
                <div class="mt-2">Price: ${formatter.format(productDetail[0].price)}</div>
                <div>Ngày Enter Warehouse: ${productDetail[0].entry}</div>
        `;
    ...
    if (ev.matches('#delete')) {
        ...
        notificationAction("Delete Product Successfully.", "#38e867");
    }
}
...
function actUpdate() {
    PRODUCTS.forEach(function (product) {
        if (product.code === code.value) {
            ...
            localStorage.setItem('DATABASE', JSON.stringify(DATABASE));
            location.reload();
            notificationAction("Successful Product Update.", "#1216e6");
        }
    })
}
    ...
    ORDERS.forEach(order => {
        ...
        if (order.status === "Order") {
            ...
            options = `
                ...
                <option value="Delivery successful" selected>Delivery successful</option>`
        }
        contents += `
            <tr>
    ...
    customer_info.innerHTML = `
        ...
        <div class="col-3">
            <div class="bg-light p-2">
                <p>Code Bill: #${order.orderId}</p>
                <p>Date created: ${order.createDate}</p>
            </div>
        </div>
        <div class="col-6">
            <p>Payment methods</p>
